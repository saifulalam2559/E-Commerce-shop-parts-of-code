...
{
    /*****************************************
   E-commerce shop products brand
  ****************************************/
    ...
    public function store(Request $request)
    {
        $this->validate(
            ...
            [
                "title.string" =>
                    "Bitte verwenden Sie eine Zeichenfolge im Titel",
        ...
        if ($status) {
            return redirect()
                ->route("brand.index")
                ->with("success", "Marke wurde erfolgreich erstellt!");
        } else {
            return redirect()
                ->back()
    ...
    public function update(Request $request, $id)
    {
        ...
        if ($band) {
            ...
            if ($status) {
                return redirect()
                    ->route("brand.index")
                    ->with("success", "Marke wurde aktualisiert!");
            } else {
                return redirect()
                    ->back()
                    ->with("error", "Etwas ist schiefgelaufen");
    ...
    public function destroy($id)
    {
        ...
        if ($band) {
            ...
            if ($status) {
                return redirect()
                    ->route("brand.index")
                    ->with("success", "Marke wurde gelöscht!!");
            } else {
